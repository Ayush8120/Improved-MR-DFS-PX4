...
def position_control():
    ...
    dataSet = np.array(list_of_setpoints9)
    numDataPoints = len(list_of_setpoints9)//3
    ...
    ax.set_ylabel('Y(t)')
    ax.set_zlabel('Z(t)')
    ax.set_title('Trajectory of 9th UAV')
    
    # Creating the Animation object
    line_ani = animation.FuncAnimation(fig, func, frames=numDataPoints + 1, fargs=(dataSet,line,redDots), interval=1500, blit=False)
    line_ani.save(r'9_th_UAV_animation.gif')
    ...
    for i in range(num_of_points):
        ...
        while not rospy.is_shutdown() and not drone_reached(list_of_setpoints9[3*i] - spawn_location[27],list_of_setpoints9[3*i+1] - spawn_location[28],list_of_setpoints9[3*i+2] - spawn_location[29]):
            ...
            local_pos_pub.publish(pose)
            #print('here!!')
            # plt.scatter(new_pose.pose.position.x/50 , new_pose.pose.position.y/50, marker = '.', color='coral')
            # plt.draw()
            rate.sleep()
...
if __name__ == '__main__':
    try:
        #print('D')
        position_control()
        # plt.savefig("9_city_uav_route_yo_final_run.png")
