...
local_pos_pub = rospy.Publisher('uav7/mavros/setpoint_position/local', PoseStamped, queue_size=1)

rospy.Subscriber('uav7/mavros/state', State, state_cb)
rospy.Subscriber('uav7/mavros/local_position/pose', PoseStamped, position_cb)

#state_sub = rospy.Subscriber(mavros.get_topic('uav1','state'), State, state_cb)
arming_client = rospy.ServiceProxy('uav7/mavros/cmd/arming', mavros_msgs.srv.CommandBool)
set_mode_client = rospy.ServiceProxy('uav7/mavros/set_mode', mavros_msgs.srv.SetMode)

pose = PoseStamped()
#Leaf node cordinates with elevation
pose.pose.position.x = spawn_location[21]             #CHANGE TODO # Hover over spawn location # personal
pose.pose.position.y = spawn_location[22]
pose.pose.position.z = spawn_location[23] + 8 
...
def position_control():
    ...
    global spawn_location
    _,_,_,_,_,_,_, list_of_setpoints7,_,_ = algorithm()
    print(list_of_setpoints7)
    num_of_points = len(list_of_setpoints7)//3
    # print(list_of_setpoints1)
    rospy.init_node('offb_node7', anonymous=True)
    prev_state = current_state
    rate = rospy.Rate(20.0) # MUST be more then 2Hz

    dataSet = np.array(list_of_setpoints7)
    numDataPoints = len(list_of_setpoints7)//3
    ...
    for i in range(num_of_points):
        ...
        if i ==2:
            ...
            ayushtrial = 1
        #substract personal
        while not rospy.is_shutdown() and not drone_reached(list_of_setpoints7[3*i] - spawn_location[21],list_of_setpoints7[3*i+1] - spawn_location[22],list_of_setpoints7[3*i+2] - spawn_location[23]):
            ...
            pose.pose.position.x = list_of_setpoints7[3*i] - spawn_location[21] #initial x #personal
            pose.pose.position.y = list_of_setpoints7[3*i+1] - spawn_location[22]#initial y
            pose.pose.position.z = list_of_setpoints7[3*i+2]  - spawn_location[23]#initial_height
            local_pos_pub.publish(pose)
            #print('here!!')
            # plt.scatter(new_pose.pose.position.x/50 , new_pose.pose.position.y/50, marker = '.', color='black')
            # plt.draw()
            rate.sleep()
...
if __name__ == '__main__':
    try:
        #print('D')
        position_control()
        # plt.savefig("7_city_uav_route_yo_final_run.png")
