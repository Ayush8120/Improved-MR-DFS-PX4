...
pose.pose.position.x = spawn_location[6] #CHANGE TODO 
pose.pose.position.y = spawn_location[7]
pose.pose.position.z = spawn_location[8] + 3
...
def position_control():
    ...
    _,_, list_of_setpoints2,_,_,_, _, _, _,_ = algorithm()
    
    num_of_points = len(list_of_setpoints2)//3
    print(list_of_setpoints2)
    rospy.init_node('offb_node2', anonymous=True)
    prev_state = current_state
    rate = rospy.Rate(20.0) # MUST be more then 2Hz

    dataSet = np.array(list_of_setpoints2)
    numDataPoints = len(list_of_setpoints2)//3
    ...
    for i in range(num_of_points):
        ...
        if i ==2:
            ...
            ayushtrial = 1
        while not rospy.is_shutdown() and not drone_reached(list_of_setpoints2[3*i] - spawn_location[6],list_of_setpoints2[3*i+1] - spawn_location[7],list_of_setpoints2[3*i+2] - spawn_location[8]):
            ...
            pose.pose.position.x = list_of_setpoints2[3*i] - spawn_location[6] #initial x
            pose.pose.position.y = list_of_setpoints2[3*i+1] - spawn_location[7]#initial y
            pose.pose.position.z = list_of_setpoints2[3*i+2] - spawn_location[8] #initial_height
            local_pos_pub.publish(pose)
            # plt.scatter(new_pose.pose.position.x/50 , new_pose.pose.position.y/50, marker = '.', color='blue')
            # plt.draw()
            #print('here!!')
            rate.sleep()
...
if __name__ == '__main__':
    try:
        #print('D')
        position_control()
        # plt.savefig("2_city_uav_route_yo_final_run.png")
