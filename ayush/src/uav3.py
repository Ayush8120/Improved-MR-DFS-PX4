...
def position_control():
    ...
    for i in range(num_of_points):
        ...
        while not rospy.is_shutdown() and not drone_reached(list_of_setpoints3[3*i]-7,list_of_setpoints3[3*i+1] - 4,list_of_setpoints3[3*i+2]):
            ...
            pose.pose.position.x = list_of_setpoints3[3*i] - 7
            pose.pose.position.y = list_of_setpoints3[3*i+1] - 4
            pose.pose.position.z = list_of_setpoints3[3*i+2]
            local_pos_pub.publish(pose)
            #print('here!!')
            rate.sleep()
