...
local_pos_pub = rospy.Publisher('uav5/mavros/setpoint_position/local', PoseStamped, queue_size=1)

rospy.Subscriber('uav5/mavros/state', State, state_cb)
rospy.Subscriber('uav5/mavros/local_position/pose', PoseStamped, position_cb)

#state_sub = rospy.Subscriber(mavros.get_topic('uav1','state'), State, state_cb)
arming_client = rospy.ServiceProxy('uav5/mavros/cmd/arming', mavros_msgs.srv.CommandBool)
set_mode_client = rospy.ServiceProxy('uav5/mavros/set_mode', mavros_msgs.srv.SetMode)

pose = PoseStamped()
#Leaf node cordinates with elevation
pose.pose.position.x = spawn_location[15]             #CHANGE TODO # Hover over spawn location # personal
pose.pose.position.y = spawn_location[16]
pose.pose.position.z = spawn_location[17] + 6 
...
def position_control():
    ...
    global spawn_location
    _,_,_,_,_, list_of_setpoints5,_,_,_,_ = algorithm()
    print(list_of_setpoints5)
    num_of_points = len(list_of_setpoints5)//3
    # print(list_of_setpoints1)
    rospy.init_node('offb_node5', anonymous=True)
    prev_state = current_state
    rate = rospy.Rate(20.0) # MUST be more then 2Hz

    dataSet = np.array(list_of_setpoints5)
    numDataPoints = len(list_of_setpoints5)//3
    ...
    ax.set_ylabel('Y(t)')
    ax.set_zlabel('Z(t)')
    ax.set_title('Trajectory of 5th UAV')
    
    # Creating the Animation object
    line_ani = animation.FuncAnimation(fig, func, frames=numDataPoints + 1, fargs=(dataSet,line,redDots), interval=1000, blit= False)
    line_ani.save(r'5_th_UAV_animation.gif')
    ...
    for i in range(num_of_points):
        ...
        if i ==2:
            ...
            ayushtrial = 1
        #substract personal
        while not rospy.is_shutdown() and not drone_reached(list_of_setpoints5[3*i] - spawn_location[15],list_of_setpoints5[3*i+1] - spawn_location[16],list_of_setpoints5[3*i+2] - spawn_location[17]):
            ...
            pose.pose.position.x = list_of_setpoints5[3*i] - spawn_location[15] #initial x #personal
            pose.pose.position.y = list_of_setpoints5[3*i+1] - spawn_location[16]#initial y
            pose.pose.position.z = list_of_setpoints5[3*i+2]  - spawn_location[17]#initial_height
            local_pos_pub.publish(pose)
            #print('here!!')
            # plt.scatter(new_pose.pose.position.x/50 , new_pose.pose.position.y/50, marker = '_', color='red')
            # plt.draw()
            rate.sleep()
...
if __name__ == '__main__':
    try:
        #print('D')
        position_control()
        # plt.savefig("5_city_uav_route_yo_final_run.png")
